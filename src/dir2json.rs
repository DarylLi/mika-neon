...
pub fn generateExport(mut cx: FunctionContext)->JsResult<JsString>{
    let mainKey = cx.argument::<JsString>(0)?.value(&mut cx);
    let entryDir = cx.argument::<JsString>(1)?.value(&mut cx);
    let outPutDir = cx.argument::<JsString>(2)?.value(&mut cx);
    // generateJsonFile("rust-umi-generate", "./react-umi", "deep.js");
    generateJsonFile(mainKey.as_str(), entryDir.as_str(), outPutDir.as_str());
    Ok(cx.string("wa chahca"))
}
// fn main() {
